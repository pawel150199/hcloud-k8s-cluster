...
flowchart TD
  ...
  class NET,SUB,M,W res;
  class O1,O2 out;
```

`depends_on` wires the subnet before the servers, and the workers after the
master, so the control plane exists before any agent tries to join.
...
sequenceDiagram
  ...
  W->>M: ssh to master and read node-token
  M-->>W: k3s join token
...
# 2. pull the join token from the master over SSH
...
> **Trust model.** Workers authenticate to the master over SSH (using an
> injected private key) to read the k3s node-token, then use that token to join
> the API server. This is why the module takes both a worker **public** and
> **private** SSH key.
